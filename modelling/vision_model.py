...
class VisionModel(nnx.Module):
    def __init__(
        ...
        #     rngs=rngs
        # )
        self.patch_unproj = nnx.Linear(
            hidden_dim,
            hidden_dim * patch_size * patch_size,
            use_bias=use_bias,
            dtype=jnp.bfloat16,
            rngs=rngs
        )
    ...
    def input_embedding(self, x, aug_puzzle_idx):
        ...
        seq_emb = seq_emb.reshape(B, self.input_size, self.input_size, -1)
        seq_emb = self.patch_proj(seq_emb)
        seq_emb = seq_emb.reshape(B, -1, D)
    ...
    def output_head(self, x):
        ...
        x = self.patch_unproj(x)
        x = x.reshape(B, self.input_size * self.input_size, -1)
